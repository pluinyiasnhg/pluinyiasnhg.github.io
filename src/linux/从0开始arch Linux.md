...
# 命令行软件 vim git zsh sudo
# 总计1.5GB
pacstrap -K /mnt base linux base-devel linux-firmware alsa-utils pipewire pipewire-alsa pipewire-pulse pipewire-jack wireplumber networkmanager bluez bluez-utils openssh ntfs-3g reflector intel-ucode vim git zsh sudo
```
...
Linux下WPS无法输入中文通常因缺少环境变量配置导致，解决方法是在WPS启动脚本（wps, et, wpp）中添加fcitx环境变量。
...
flatpak wps启动后，wpscloudsvr 总是崩溃，解决方法是将这个云服务功能关闭。具体操作如下：

```zsh
# 找到 wps 安装位置
flatpak info --show-location com.wps.Office

# 重命名 wpscloudsvr
sudo mv /var/lib/flatpak/app/com.wps.Office/x86_64/stable/[一串数字]/files/extra/wps-office/office6/wpscloudsvr /var/lib/flatpak/app/com.wps.Office/x86_64/stable/[一串数字]/files/extra/wps-office/office6/wpscloudsvr.bak
```
