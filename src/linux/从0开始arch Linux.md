...
不过遇到个问题：参考官方文档中[自定义样式表](https://xiaoyifang.github.io/goldendict-ng/topic_userstyle/)，一款主题由两个文件构成，分别是article-style.css 和 qt-style.css 。实际操作下来，qt-style.css 没有生效，始终是晃眼的白色。
...
- 记忆 Anki
- 文献管理 Zotero
- 应用商店 Sparkstore（arch不能用）
- RustDesk：跨平台远程桌面。（没用过）
- Rclone：云端硬盘备份。（没用过）
- Ventoy：多系统安装。很奇怪，用了这个软件后我的u盘几乎没有存资料的空间，不知道哪里搞错了。
- 乐谱软件 MuseScore（没用过）
