...
学习尚硅谷的[《MySQL数据库入门到大牛，mysql安装到优化》](https://www.bilibili.com/video/BV1iq4y1u7vj)的第12到48分集。
...
```sql
# 单行函数支持嵌套使用
mysql> SELECT TRUNCATE(ROUND(123.456,2),0);
+------------------------------+
...
> [!info] 如何需要统计表中的记录数，使用`COUNT(*)`、`COUNT(1)`、`COUNT(具体字段)` 哪个效率更高呢？
...
GROUP BY中使用`WITH ROLLUP`。在所有查询出的分组记录之后增加一条记录，该记录计算查询出的所有记录的总和，即统计记录数量。
...
SELECT 语句的执行顺序：`FROM -> ON -> JOIN -> WHERE -> GROUP BY -> HAVING -> SELECT 的字段 -> DISTINCT -> ORDER BY -> LIMIT` 。
