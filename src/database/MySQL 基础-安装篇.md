...
> 我已放弃使用，DBeaver 用起来很奇怪。而且社区版没法连接Redis。
...
问题描述：在使用 DataGrip 编写 SQL 时，自动补全字段名（如 `employee_id`）会自动带上表名（如 `employees.employee_id`），导致代码冗余。

解决方法：

1. 进入设置：文件 -> 设置 -> 编辑器 -> 常规 -> 代码补全 -> 弹出窗口
2. 修改配置：在右侧面板找到【对象限定范围】，将【始终】改为【从不】
